...
### Remove the plugin name field

Once you've set up a sidecar plugin that handles the same files as the legacy
plugin (including the discover property so that argo-cd will know to select it),
you should update each Application spec to remove the plugin name.

E.g., if you had an application that looked like:
...
spec:
  source:
    plugin:
      name: pluginName  # Delete this line
```
