...
```
      ...
      --content-security-policy value                 Set Content-Security-Policy header in HTTP responses to value. The default is default-src 'self'; script-src 'self' 'unsafe-eval'; style-src 'self' 'unsafe-inline'. To disable, set to "". (default "default-src 'self'; script-src 'self' 'unsafe-eval'; style-src 'self' 'unsafe-inline'")
