...
func getParamSetsByMergeKey(mergeKeys []string, paramSets []map[string]interface{}, useGoTemplate bool, goTemplateOptions []string) (map[string]map[string]interface{}, error) {
	...
	for i, paramSet := range paramSets {
		...
		paramSetKey, err := generateParamSetValue(deDuplicatedMergeKeys, paramSet, useGoTemplate, goTemplateOptions)
		if err != nil {
			return nil, fmt.Errorf("failed generating paramSetKey for paramSet %v: %w", i, err)
		}
...
// generateParamSetValue uses the keys to generate a stable value for a paramSet.
//
// This can be thought of a modulo operation: 'paramSet % keys' leaves a
// representation of the paramSet with respect to the keys.
func generateParamSetValue(keys map[string]bool, paramSet map[string]interface{}, useGoTemplate bool, goTemplateOptions []string) (paramSetValue map[string]interface{}, err error) {
	paramSetValue = make(map[string]interface{})
	...
	for mergeKey := range keys {
		if useGoTemplate {
			...
			paramSetValue[mergeKey] = templatedMergeKey
		} else {
			paramSetValue[mergeKey] = paramSet[mergeKey]
		}
	}

	return paramSetValue, nil
}
