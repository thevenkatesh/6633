...
const (
	...
	appHealthStatusResyncDuration           = 30 * time.Second
...
func (ctrl *ApplicationController) processAppRefreshQueueItem() (processNext bool) {
	...
	if comparisonLevel == ComparisonWithNothing {
		...
		if err := ctrl.cache.GetAppManagedResources(app.InstanceName(ctrl.namespace), &managedResources); err != nil {
			...
			app.Status.Health.ObservedAt = &metav1.Time{Time: time.Now()}
	...
	if hasErrors {
		...
		app.Status.Health.ObservedAt = &now
		patchMs = ctrl.persistAppStatus(origApp, &app.Status)
...
func (ctrl *ApplicationController) newApplicationInformerAndLister() (cache.SharedIndexInformer, applisters.ApplicationLister) {
	...
	// Periodically updates ObservedAt time in Application health status to indicate freshness
	_, err = informer.AddEventHandlerWithResyncPeriod(
		cache.ResourceEventHandlerFuncs{
			UpdateFunc: func(old, new interface{}) {
				...
				newApp, newOK := new.(*appv1.Application)
				if oldOK && newOK {
					// To prevent a status update hot loop, enqueue only events generated by resyncs
					// and ignore those triggered by application updates, which are handled by another event handler.
