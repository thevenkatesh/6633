...
func NewCommand() *cobra.Command {
	...
	command.Flags().StringVar(&contentSecurityPolicy, "content-security-policy", env.StringFromEnv("ARGOCD_SERVER_CONTENT_SECURITY_POLICY", "default-src 'self'; script-src 'self' 'unsafe-eval'; style-src 'self' 'unsafe-inline'"), "Set Content-Security-Policy header in HTTP responses to `value`. The default is default-src 'self'; script-src 'self' 'unsafe-eval'; style-src 'self' 'unsafe-inline'. To disable, set to \"\".")
